...
function []=displayMeanMapsAlpha(map,Fs,xlab,ylab,ylimits,nx,ny,xlimits,imageFontSize,imageSize,colorbarLabel,colorMap,units,effectNames)
...
Co.Label.String=[colorbarLabel sprintf('\n') ' for 1' units ' of ' lower(effectNames)];
Co.FontSize=imageFontSize;


maxMap=max([abs(max(max(map))) abs(min(min(map)))]);
caxis([-maxMap maxMap])
